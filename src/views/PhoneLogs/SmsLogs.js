...
const SmsLogs = () => {
  ...
  } = useCommonCheckbox(data, "smsLogId");
  const handleDelete = async (
    ...
    const selectedName = data
      ?.filter((item) => checkedData.includes(item["smsLogId"])) // Filter items with matching id
      .map((item) => item?.number); // Extract the name field
  ...
  const columns = [
    {
      ...
      renderCell: (params) => (
        <div>
          <Checkbox
            type="checkbox"
            checked={childCheckedState?.includes(params?.row?.smsLogId)}
            onChange={(e) =>
              handleChildCheckboxChange(e, params?.row?.smsLogId)
            }
          />
        </div>
      ),
