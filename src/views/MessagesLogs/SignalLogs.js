...
const SignalLogs = () => {
  ...
  const handleDelete = async (checkedData, setParentChecked, setChildCheckedState) => {
    const selectedName = data
      ?.filter((item) => checkedData.includes(item["contactNumber"]))
        ...
        <div>
          <Button className="link-redirect">
            <FaTrash
              color="red"
              stroke={2}
              onClick={() => handleDelete([params?.row?.contactNumber], setParentChecked, setChildCheckedState)}
            />
          </Button>
        </div>
      ),
                ...
                {chatData.length > 0 &&
                  chatData.map((chat) => {
                    return (
                      <>
                        <div
                          className={chat.messageType === "Incoming" ? "msg left-msg" : "msg right-msg"}
                        >
