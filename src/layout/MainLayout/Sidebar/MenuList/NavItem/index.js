...
import { menuOpen } from "store/reducers/customizationSlice";
...
const NavItem = ({ item, level }) => {
      ...
      if (params?.userDeviceId) {
        return (
          <Link
            ref={ref}
            {...props}
            to={item.url + "/" + params.userDeviceId}
            target={itemTarget}
          />
        );
      }
  ...
  return (
    <ListItemButton
      // className="aside-item-active active"
      className={
        pathname === item.url
          ? "aside-item-active active"
          : pathname.substring(0, pathname.lastIndexOf("/")) === item.url
          ? "aside-item-active active"
